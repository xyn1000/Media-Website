...
def single_podcast(podcast_id):
    ...
    page['title'] = 'Podcast Details Page'  # Add the title

    # Set up some variables to manage the returns from the database fucntions

    result = database.get_podcast(podcast_id)
...
def single_podcastep(media_id):
    ...
    # Set up some variables to manage the returns from the database fucntions
    result = database.get_podcastep(media_id)
...
def search_movies():
    ...
    page['title'] = 'Movie Show Search'  # Add the title
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        song = database.add_song_to_db(newdict['song_title'], newdict['song_length'], newdict['storage'],
...
@app.route('/search/power', methods=['POST','GET'])
def power_search():
    """
    Extension: power search with fuzzing functions
    """
    # Check if the user is logged in, if not: back to login.
    if 'logged_in' not in session or not session['logged_in']:
        return redirect(url_for('login'))
    ...
    page['title'] = 'Power Search'  # Add the title

    movies = None
    if (request.method == 'POST'):
        search_term = request.form['searchterm']
        movies = database.movie_power_search(search_term)
        songs = database.song_power_search(search_term)
        podcasts = database.podcast_power_search(search_term)
        tvshows = database.tvshow_power_search(search_term)
    ...
    total_num = len(songs) + len(podcasts) + len(tvshows) + len(movies)

    if total_num > 0:
        page['bar'] = True
        flash('Found ' + str(total_num) + ' results!')
    else:
        page['bar'] = False
        flash("No matching movies found, please try again")

    return render_template('searchitems/power_search.html',
                           session=session,
                           page=page,
                           user=user_details,
                           movies=movies,
...
@app.route('/search/allname', methods=['POST','GET'])
def search_all():
    """
